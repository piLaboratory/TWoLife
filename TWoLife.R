...
frag<- function(directory,TNW =1 , numb.cells = 100, cell.size = 1, land.shape = 1, type=c("random","blob"), bound.condition=0, cover=1){
  
  library(vegan)
  
  land<- Landscape(numb.cells, cell.size, land.shape, type, bound.condition, cover)
  temp<- numeric(length(land$scape))
  ...
  for (i in 1:length(files)) {
    ...
    frag=read.table(files[i])
    
    count=0
    for (j in 1:sqrt(length(land$scape))) {
      for (k in 1:sqrt(length(land$scape))) {
        count = count +1
        temp[count]= temp[count]+frag[j,k]
      }
    }
  }
  
  #land$scape<-decostand(temp, method = "range")
  land$scape<-1-(decostand(temp, method = "range")*(TNW))
